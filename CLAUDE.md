...
- `PermissionHealthCheck` verifies each TCC permission by combining the system verdict with a live probe. Each resolves to `PermissionStatus` (`.healthy | .denied | .broken | .notDetermined`). `.broken` means TCC says allowed but the probe disagrees — fix is to toggle the permission off and on in System Settings. **A probe that fails must first be shown to be about us.** The Accessibility probe asks the system-wide AX element for the *frontmost app's* focused application, so most of its errors describe that app, not our grant; treating every non-success as `.broken` produced a notification telling users to toggle a working permission, with the verdict flipping healthy → broken → healthy inside 70 seconds in one process with no user action. Only `kAXErrorAPIDisabled` is evidence about our access (`AXIsProcessTrusted()` is already the system's own answer, and `.broken` exists only for the contradiction between the two); everything else is `AccessibilityProbe.inconclusive` and reports healthy. `kAXErrorNotImplemented` is deliberately NOT grouped with it — Apple documents it as the asked *process* lacking AX support. The raw `AXError` goes into the diagnostic log, because the old `Bool` threw away the one fact that explained the false alarm.
