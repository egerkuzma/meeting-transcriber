...
app/MeetingTranscriber/    # Swift macOS menu-bar app (SPM)
  ...
  Sources/                 # @main app shell + AppState composition root wiring the concern
                           ...
                           #   + GigaAMEngine (Russian only, via sherpa-onnx)
                           #   + WhisperCppEngine (ggml fine-tunes, via whisper.cpp).
...
- `TranscribingEngine` protocol abstracts ASR backends. Four implementations: `WhisperKitEngine` (99+ languages, ~1 GB model), `ParakeetEngine` (25 EU languages, ~50 MB model, ~10× faster), `GigaAMEngine` (Sber GigaAM-v3 e2e-RNNT, Russian only, via sherpa-onnx) and `WhisperCppEngine` (any ggml Whisper fine-tune, via whisper.cpp).
- `AppSettings.transcriptionEngine` enum (`.whisperKit` / `.parakeet` / `.gigaam` / `.whisperCpp`) selects the engine. Settings UI shows engine picker; engine-specific options hidden when not selected. `availableCases` (filtered by `isAvailable`) is the picker source — a capability hook kept for engines with stricter OS floors.
...
**GigaAM (Russian):** `GigaAMEngine` drives the Sber GigaAM-v3 e2e-RNNT ONNX graphs through sherpa-onnx's offline transducer recognizer (`nemo_transducer`, feature_dim 64 — *not* sherpa's default 80, and a mismatch there yields plausible-looking wrong text rather than an error).
- **It is the one engine the app does not download.** The four artefacts are installed by hand under `AppPaths.gigaamModelDir` (`~/Library/Application Support/MeetingTranscriber/models/gigaam-v3/`), so a missing folder is a normal user-actionable state, not a failure: `GigaAMModelFiles.resolve` returns a message naming the path and the engine reports `EngineModelState.failed(message)`. That case exists for this reason — `.unloaded` would read as "not tried yet" and send the user looking for a download that never happens. `wireName` keeps the RPC strings ("unloaded"/…/"failed") from carrying the path.
- **Long audio is split before it reaches the recognizer.** The graph decodes a whole utterance in one pass with no state carried across calls, so `GigaAMChunking` cuts at the quietest 100 ms window between 15 s and 20 s — a pause wherever the audio offers one, the least-loud point otherwise. Each chunk becomes one `TimestampedSegment`, so timestamps are as coarse as 20 s: diarization still runs (it reads the audio itself and maps by timestamp) but a chunk spanning two speakers is attributed to one. The transducer result does carry per-token timestamps, so a finer split is available later without changing the engine's interface.
- `SherpaOnnxOfflineRecognizer` is a non-`Sendable` class from a Swift-5-mode module and its construction compiles the ONNX graphs, so `GigaAMRecognizer` is an `actor` that confines it and builds it in `load()` rather than `init` — an actor's non-async `init` runs on the *caller's* thread, which would put the compile back on the main actor.
- No live transcription (`supportsLiveTranscription == false`, `StreamingTranscribingEngine` not adopted). Captions are not lost: the engine reports `ru`, and a set language routes them to the engine-independent Nemotron streaming session.
...
**Whisper.cpp (ggml fine-tunes):** `WhisperCppEngine` runs ggml Whisper models through the prebuilt whisper.cpp xcframework (`WhisperFramework`, b4938, Metal). It exists for *fine-tunes*: WhisperKit runs Apple's CoreML conversions of the stock checkpoints, and a community fine-tune ships as a ggml `.bin` nothing else here can load. Both the model path (`AppSettings.whisperCppModelPath`, default `models/whisper-cpp/ggml-large-v3-russian.bin`) and the language (`whisperCppLanguage`, default `ru`) are settings, because which model is installed decides what language it should be told — and a monolingual fine-tune given the wrong code mistranscribes rather than failing.
- **No chunking**, unlike GigaAM: whisper.cpp does its own 30 s windowing with context carried across windows, so the whole buffer goes into one `whisper_full` call and the segments come back phrase-shaped. Verified end-to-end on `two_speakers_de.wav`: per-phrase timestamps that diarization maps cleanly onto alternating speakers.
- **`WhisperCppContext` is a class over a serial `DispatchQueue`, not an `actor`** (the one structural difference from `GigaAMRecognizer`). `whisper_full` blocks for the length of the transcription, and an actor body runs on a cooperative thread-pool thread where a block that long starves the pool. The queue also supplies the serialization whisper.cpp requires — it documents `whisper_full` as "not thread safe for same context", which actor reentrancy would not guarantee. The context is loaded once and reused (a 3 GB model takes seconds to map), freed in `deinit`.
- Two C-bridging details that fail silently if got wrong: `params.language` stores the pointer rather than copying, so the string must outlive the call; and the progress callback is a capture-less C function pointer, so the Swift closure travels through `progress_callback_user_data` in an `Unmanaged` box kept alive with `withExtendedLifetime`. The library's own logging is routed into os.log via `whisper_log_set` — otherwise a load error lands only on stderr, where a shipped app has no reader.
- Like GigaAM the app downloads nothing, and a missing file resolves to `EngineModelState.failed` naming the path. `supportsLiveTranscription` is false; captions still work through the language-driven Nemotron streaming session.
- **The custom vocabulary file is deliberately NOT wired to `initial_prompt`, and should stay that way.** whisper.cpp exposes the parameter and the plumbing is a couple of hours' work, so the idea recurs; it was built, measured on a five-minute slice of a real Russian meeting against the same model with the toggle as the only variable, and reverted. Three prompt shapes, three failures, measured against a 63-segment / 786-word baseline: a bare comma-separated term list produced 210 segments and 1670 words at 0.002 similarity, one invented sentence repeated **104 times**; a natural sentence carrying the terms held together at 0.699 similarity but still lost 14% of the words and came back lowercase and unpunctuated; and — the decisive one — a neutral control sentence naming *none* of the vocabulary ("two friends are talking about food and drink") looped **93 times**. So the damage is not the term list, the phrasing, or the format: it is the presence of a prompt at all on this audio. Terms the model recognised *without* a prompt (Кроненбург, Перекресток, апдейт) disappeared *with* one. Not established: whether this is whisper.cpp's own prompt behaviour or a defect in the reverted bridging — settling that needs the upstream `whisper-cli` run against the same model and audio, which this project does not build (it links the xcframework only). The measured facts stand either way. `TerminologyNormalizer` is the mechanism that does work for this, applying `Canonical => variant` rules after ASR where nothing can be lost.
- **A repetition detector that scans for adjacent repeated 4- and 5-grams does not catch this.** The loop above cycles roughly fifteen words and repeats a whole segment later, so that check reported the affected transcript as clean. Any future "is this transcript sane" heuristic has to count repeated *segments*, not neighbouring n-grams.
- **`whisper.framework` is the only *dynamic* binary dependency in the project, and the bundle assemblers have to carry it.** SwiftPM stages it next to the product in `.build/<config>/`, which the default `@loader_path` rpath covers, so `swift build`/`test`/`run` need nothing — but both assemblers copy only the executable, and merely adding the binary target to the target's dependencies (before one Swift file imported it) was already enough to make the assembled bundle die at launch with "Library not loaded". `scripts/lib/whisper-framework.sh` copies it into `Contents/Frameworks` for both `run_app.sh` and `build_release.sh`, and `Package.swift` adds the matching `@executable_path/../Frameworks` rpath. Neither half works alone. The notarization path signs the framework bundle explicitly: the existing `find -name "*.dylib" -o -name "*.so"` cannot reach a framework whose Mach-O is named `whisper`.
